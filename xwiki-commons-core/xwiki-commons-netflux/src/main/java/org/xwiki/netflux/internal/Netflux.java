...
import org.xwiki.netflux.internal.event.SendChannelMessageEvent;
...
public class Netflux
{
    ...
    private void leaveChannel(LocalUser user, Channel channel, String reason)
    {
    ...
    /**
     * @param user the local user to send to the message to
     ...
     */
    private void sendChannelMessage(String cmd, LocalUser sender, Channel channel, String message)
    {
        // Send commands through events instead of directly so that other cluster members receive it too
        this.observation.notify(new SendChannelMessageEvent(cmd, channel.getKey()), sender, message);
    }
}
