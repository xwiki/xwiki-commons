...
import org.xwiki.netflux.internal.event.NetfluxPingUserEvent;
import org.xwiki.netflux.internal.event.NetfluxUserJoinedEvent;
import org.xwiki.netflux.internal.event.NetfluxUserLeftEvent;
...
public class Netflux
{
    ...
    public void onClose(Session session, CloseReason closeReason)
    {
        synchronized (this.bigLock) {
            ...
            new LinkedList<Channel>(user.getChannels()).forEach(channel -> this.observation
                .notify(new NetfluxUserLeftEvent(user.getName(), channel.getKey(), "Disconnected"), null));
        }
    }
    ...
    private LocalUser getOrRegisterUser(Session session)
    {
        ...
        if (user == null) {
            ...
            user = new LocalUser(session, userId);
    ...
    private void onCommandJoin(LocalUser user, Integer seq, String channelKey)
    {
        ...
        this.observation.notify(new NetfluxUserJoinedEvent(seq, user.getName(), channelKey), null);
    }
    ...
    private void onCommandLeave(LocalUser user, Integer seq, String channelKey)
    {
        ...
        this.observation.notify(new NetfluxUserLeftEvent(user.getName(), channelKey, null), null);
    }
    ...
    private void onCommandPing(LocalUser user, Integer seq)
    {
        this.observation.notify(new NetfluxPingUserEvent(user.getName(), seq), null);
    }
    ...
    private void onCommandMessage(LocalUser user, Integer seq, String channelKeyOrUserName, List<Object> msg)
    {
        ...
        if (bot.isPresent()) {
            ...
            String msgMsg = this.messageBuilder.buildMessage(0, user.getName(), channelKeyOrUserName, msg.get(3));
            this.observation.notify(new NetfluxMessageUserEvent(user.getName(), msgMsg), null);
