...
// @formatter:on
class BcDESKeyGeneratorTest
{
    @InjectMockComponents
    private BcDESKeyGenerator generator;
    ...
    void generateWithoutArgument()
    {
        ...
        assertThat(key.length, equalTo(8));
    ...
    {
        KeyGenerationParameters params = new GenericKeyGenerationParameters(7);
        byte[] key = generator.generate(params);

        assertThat(key.length, equalTo(8));
        assertThat(key, not(equalTo(generator.generate(params))));
        assertThat(generator.generate(new GenericKeyGenerationParameters(8)).length, equalTo(8));
    }
}
